...
6. **Pull Initial Ollama Model**
   - After starting the stack (make sure homie_llm has been started), run:
     ```bash
     docker exec homie_llm ollama pull modelname
     ```
   - this will download the `modelname` into the homie_llm container
   ...
   - In case of a cloud model it will just make it available without a download.
     ...
     ```bash
     docker exec homie_llm ollama signin
     ```
